...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'training.json',  ###Hieunt - warninng update path 
        img_prefix=data_root,
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + '.json', ###Hieunt - warninng update path 
        img_prefix=data_root + 'val2017/',
        pipeline=test_pipeline),
    test=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/instances_val2017.json', ###Hieunt - warninng update path 
        img_prefix=data_root + 'val2017/',
        pipeline=test_pipeline))
